...
import { SET_COMMENT_REPLY_3 } from "../../redux/types/dataTypes";
import Replies4 from "./Replies4";
import axios from "axios";
import { analytics } from "../../firebase";

function ReplyContent3({
  body,
  createdAt,
  username,
  comments,
  ...
  const useStateParameters = () => {
    ...
    return useSelector(
      (state) => ({
        UIErrors: state.UI.commentReplyFormErrors3,
  ...
  const handleReplyFormClose = () => {
    ...
    setFormBody("");
    dispatch({ type: CLEAR_COMMENT_REPLY_FORM_ERRORS_3 });
  };
  ...
  const handleSubmit = (event) => {
    ...
    axios
      ...
      .then((res) => {
        dispatch({ type: SET_REPLY_STATUS_3 });
        dispatch({ type: SET_COMMENT_REPLY_3, payload: res.data });
        dispatch({ type: CLEAR_COMMENT_REPLY_FORM_ERRORS_3 });
      ...
      .catch((err) => {
        ...
        // clear errors
        dispatch({
          type: SET_COMMENT_REPLY_FORM_ERRORS_3,
  ...
  const repliesToReply = comments > 0 ? <Replies4 commentId={replyId} /> : null;
